...
import xjunz.tool.wechat.databinding.FragmentAdvancedEditorBinding;
import xjunz.tool.wechat.databinding.ItemAdvancedEditorBinding;
...
import xjunz.tool.wechat.impl.util.ContentType;
    ...
    private class EditorViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(@NotNull View v) {
            ...
            if (Message.KEY_LV_BUFFER.equals(item.key)) {
                new LvBufferEditorDialog().setTypeSerial(Message.LV_BUFFER_READ_SERIAL).setDefault(mVictim.getParsedLvBuffer()).setCallback(item::setValue).show(requireFragmentManager(), item.key);
                return;
            }
            ...
            if (contentType == ContentType.INTEGER || contentType == ContentType.LONG) {
                ...
                }).setLabel(item.getKey()).setDefault(item.getContentPreview()).setCallback(result -> {
                    if (contentType == ContentType.BLOB) {
