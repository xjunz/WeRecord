...
import xjunz.tool.wechat.databinding.FragmentSimpleEditorBinding;
import xjunz.tool.wechat.databinding.ItemPreviewBinding;
import xjunz.tool.wechat.databinding.ItemSeparatorBinding;
import xjunz.tool.wechat.databinding.ItemSimpleEditorBinding;
...
import xjunz.tool.wechat.impl.model.message.AppMessage;
import xjunz.tool.wechat.impl.model.message.CallMessage;
import xjunz.tool.wechat.impl.model.message.ComplexMessage;
import xjunz.tool.wechat.impl.model.message.Message;
import xjunz.tool.wechat.impl.model.message.PlainMessage;
import xjunz.tool.wechat.impl.model.message.SystemMessage;
import xjunz.tool.wechat.impl.model.message.UnpreviewableMessage;
...
import xjunz.tool.wechat.ui.base.EditorFragment;
import xjunz.tool.wechat.ui.base.SingleLineEditorDialog;
...
import xjunz.tool.wechat.ui.message.fragment.dialog.TemplateShowcaseDialog;
import xjunz.tool.wechat.ui.message.fragment.dialog.TimestampEditorDialog;
import xjunz.tool.wechat.util.Returnable;
        ...
        public void onClick(@NotNull View v) {
            ...
            if (v.getId() == R.id.btn_reset) {
                if (item.key.equals(Message.ABSTRACT_KEY_SENDER_ID)) {
                    changeSenderConsideringStatus(item, mOrigin.requireSenderId());
                } else {
                    item.reset();
                }
                return;
            ...
            switch (item.key) {
                ...
                case CallMessage.ABSTRACT_KEY_CALL_CONTENT:
                    new ContentEditorDialog().setLabel(item.caption)
                            ...
                            .setCallback(item::setValue).show(requireFragmentManager(), item.key);
                    break;
                ...
                case Message.KEY_CREATE_TIME:
                    new TimestampEditorDialog().setDefault(mVictim.getCreateTimeStamp()).setCallback(item::setValue).show(requireFragmentManager(), item.key);
                    break;
                    ...
                    switch (type) {
                        case Template.RepGroup.TYPE_STRING:
                            new ContentEditorDialog().setLabel(repGroup.name).setDefault(Utils.stringValueOf(value))
                                    .setCallback(item::setValue).show(requireFragmentManager(), item.caption);
                            break;
                        case Template.RepGroup.TYPE_DECIMAL:
                            ...
                            }).setDefault(Utils.stringValueOf(value)).setCallback(result -> {
                                try {
                        ...
                        case Template.RepGroup.TYPE_LONG:
                            ...
                            break;
                        case Template.RepGroup.TYPE_TIMESTAMP:
                            new TimestampEditorDialog().setDefault(mVictim.getCreateTimeStamp()).setAllowUnchanged(true)
                                    .setCallback(item::setValue).show(requireFragmentManager(), item.caption);
                            break;
                        case Template.RepGroup.TYPE_ACCOUNT_ID:
                        case Template.RepGroup.TYPE_ACCOUNT_NAME:
                        ...
                        case Template.RepGroup.TYPE_APP_ID:
                            PopupMenu menu = new PopupMenu(requireContext(), itemView, Gravity.CENTER);
